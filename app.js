...
function save() {
  try {
    ...
    const stateWithoutWallpapers = {
      ...state,
      tabs: state.tabs.map(tab => ({ ...tab, wallpaper: null }))
    };
    ...
    if (typeof chrome !== 'undefined' && chrome.storage) {
      ...
      state.tabs.forEach(tab => {
        if (tab.wallpaper) wallpaperData[`wp_${tab.id}`] = tab.wallpaper;
      });
      if (Object.keys(wallpaperData).length) {
...
function renderWallpaper() {
  ...
  if (tab?.wallpaper) {
    ...
    // Default: knight wallpaper bundled with extension
    wp.style.backgroundImage    = `url('wallpaper-default.jpg')`;
...
function applyWallpaper(dataUrlOrSrc) {
  ...
  if (dataUrlOrSrc.startsWith('data:')) {
    ...
    img.onload = () => {
      ...
      // Max 1920px wide to keep under storage limits
      const maxW = 1920;
